...
class ControllerExtensionModulePavFlashSale extends Controller {
	...
	public function finder( $config ) {
		...
		$data['sorts'][] = array(
			...
			'value' => 'p.price-ASC',
			'href'  => $this->url->link('extension/module/pavflashsale', 'sort=p.price&order=ASC' . $url)
		);

		$data['sorts'][] = array(
			'text'  => $this->language->get('text_price_desc'),
			'value' => 'p.price-DESC',
			'href'  => $this->url->link('extension/module/pavflashsale', 'sort=p.price&order=DESC' . $url)
		);
